...
export class PurchaseRequestFactory extends ListingFactory {

  private vitalProperties : string[] = ['condition'];
   ...
   * @return {PurchaseRequest} a new instance of SaleOffer with all properties set accordingly to the body object
   * @throws {Error} an error if some properties are missing
   */
  public createListing(body : any) : PurchaseRequest {
    if (this.checkProperties(body)) {
      let listing : PurchaseRequest = new PurchaseRequest();
   ...
   */
  protected assignProperties(listing : PurchaseRequest, body : any) : void {
