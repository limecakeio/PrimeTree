...
export class ServiceRequestFactory extends ListingFactory {
  ...
  /**
   * creates a ServiceRequest based on the properties from the param object
   * @param {any} body an instance of object which holds all properties
   * @return {ServiceRequest} a new instance of ServiceRequest with all properties set accordingly to the body object
   * @throws {Error} an error if some properties are missing
   */
  public createListing(body : any) : ServiceRequest {
    if (this.checkProperties(body)) {
      let listing : ServiceRequest = new ServiceRequest();
      this.assignProperties(listing, body);
      return listing;
    } else {
      throw new Error('Cannot create ServiceRequest from body: ' + this.findMissingProperties(body).join(', ') + ' are missing.');
    }
  }

  /**
   * Assigns all properties for a ServiceRequest from body to listing.
   */
  protected assignProperties(listing : ServiceRequest, body : any) : void {
